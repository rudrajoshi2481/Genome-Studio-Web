...
import { Zap, Server, Bot, Terminal, BookOpen, Cpu, CheckCircle2, XCircle, Loader2, Database, CheckCheck, Save, Search, Link2, RotateCcw, Wifi, WifiOff, Shield, ShieldCheck, ShieldAlert, Trash2, AlertTriangle, Settings } from "lucide-react"
...
function ChatFeaturesDialog({ children, tooltipText = "AI Chat Settings" }: ChatFeaturesDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[900px] w-[900px] max-h-[90vh] h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
        ...
        {!loading && !error && (
          <Tabs defaultValue="providers" className="flex-1 min-h-0 flex flex-col">
            <TabsList className="mx-5 mt-3 grid grid-cols-5 h-9">
              ...
              <TabsTrigger value="databases" className="text-xs gap-1.5">
            ...
            <ScrollArea className="flex-1 min-h-0">
              <div className="px-5 py-4">
                      ...
                      {providers.map((p) => (
                        <div
                          ...
                          className={`flex items-center justify-between p-2.5 rounded-lg border transition-all
                            ${p.available ? "border-green-500/50 bg-green-50/50 dark:border-green-800/50 dark:bg-green-950/20" : "border-muted"}`}
                        >
                          <div className="flex items-center gap-2.5">
                            <Cpu className="h-3.5 w-3.5 text-muted-foreground" />
                            <div>
                              <p className="font-medium text-xs">{p.name}</p>
                              <p className="text-[11px] text-muted-foreground font-mono">{p.base_url}</p>
                            </div>
                          </div>
                          {p.available ? (
                            ...
                            <Badge variant="outline" className="text-[10px] text-red-500 gap-1">
                ...
                {/* ─── Databases Tab ─── */}
                <TabsContent value="databases" className="mt-0 space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-muted-foreground">
                      {enabledDatabases.length} of {databases.length} enabled
                    </p>
                    {databases.length > 0 && (
                  ...
                  {databases.length === 0 ? (
                    <p className="text-xs text-muted-foreground py-4 text-center">No databases available.</p>
                  ) : (
                    <>
                      ...
                      {databases.filter(db => {
                        ...
                        <p className="text-xs text-muted-foreground py-4 text-center">No databases match "{dbSearch}".</p>
                      ) : (
                          ...
                          {databases.filter(db => {
                            ...
                            return (
                              <div
                                key={db.id}
                                className={`flex items-center justify-between p-2.5 rounded-lg border transition-all
                                  ${active ? "border-green-500/50 bg-green-50/50 dark:border-green-800/50 dark:bg-green-950/20" : "hover:border-muted-foreground/20"}`}
                              >
                                <div className="min-w-0 flex-1 mr-2">
                                  <p className="font-medium text-xs">{db.name}</p>
                                  <p className="text-[11px] text-muted-foreground truncate">{db.description}</p>
                      ...
                      {knowledgeStats ? (
                        <div className="space-y-2">
                          ...
                          {instructions.length > 0 && (
                            <div className="mt-1">
                              <p className="text-[11px] text-muted-foreground mb-1">Instruction files:</p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      className={`flex items-start gap-2.5 p-3 rounded-lg border text-left transition-all ${
                        permissionMode === 'bypass'
                          ? 'border-green-500/50 bg-green-50/50 dark:border-green-800/50 dark:bg-green-950/20'
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">Auto-approve all tool calls. No confirmation prompts.</p>
                      </div>
                    </button>
                    <button
                      className={`flex items-start gap-2.5 p-3 rounded-lg border text-left transition-all ${
                        permissionMode === 'default'
                          ? 'border-amber-500/50 bg-amber-50/50 dark:border-amber-800/50 dark:bg-amber-950/20'
                          : 'border-muted hover:border-muted-foreground/30'
                      }`}
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">Ask before destructive tools (write, edit, run_command).</p>
                      </div>
                    </button>
                  </div>
