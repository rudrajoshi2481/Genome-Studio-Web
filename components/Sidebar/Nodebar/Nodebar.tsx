...
import { RefreshCcw, Search, Star, X, Filter, ChevronDown, ChevronUp, ArrowDownUp, Plus, Settings, Type, Hash, ToggleLeft, List as ListIcon, Braces, Workflow, Dna, Layers, Atom, Boxes } from 'lucide-react'
...
import { GENOME_BROWSER_SOURCE, GENOME_BROWSER_NODE, NGL_VIEWER_SOURCE, NGL_VIEWER_NODE } from "@/lib/builtin-nodes"
...
function Nodebar() {
  ...
  const [isProteinsOpen, setIsProteinsOpen] = useState(true)
  ...
  return (
    <div className="h-[calc(100vh-56px)] flex flex-col border-r border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-medium text-indigo-700">Genome Browser (HiGlass)</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Proteins Nodes Section — NGL structure viewer */}
      <Collapsible open={isProteinsOpen} onOpenChange={setIsProteinsOpen} className="border-b border-gray-200 bg-gray-50">
        <CollapsibleTrigger asChild>
          <Button
            variant="ghost"
            className="w-full justify-between px-3 py-2 h-auto hover:bg-accent/50 rounded-none"
          >
            <h4 className="text-xs font-semibold text-gray-600">PROTEINS</h4>
            {isProteinsOpen ? (
              <ChevronUp className="h-3 w-3 text-muted-foreground" />
            ) : (
              <ChevronDown className="h-3 w-3 text-muted-foreground" />
        ...
        <CollapsibleContent>
          <div className="p-2 pt-0">
            <div className="grid grid-cols-2 gap-2">
              {/* NGL Structure Node (dataType — zero execution, config form on canvas) */}
              <div
                draggable
                onDragStart={(e) => {
                  e.dataTransfer.setData('application/reactflow', JSON.stringify({
                    type: 'dataType',
                    dataType: 'ngl-structure',
                    label: 'Structure'
                  }));
                  e.dataTransfer.effectAllowed = 'move';
                }}
                className="p-2 border rounded cursor-move hover:shadow-md transition-all bg-rose-500/10 border-rose-500/20 hover:border-rose-500/40"
                style={{
                  backgroundImage: 'repeating-linear-gradient(135deg, transparent, transparent 6px, rgba(244, 63, 94, 0.08) 6px, rgba(244, 63, 94, 0.08) 12px)',
                }}
                title="NGL structure config: file path, file type, representation, color scheme, options. Connect to NGL Viewer."
              >
                <div className="flex items-center gap-1.5">
                  <Boxes className="h-3.5 w-3.5 text-rose-700 shrink-0" />
                  <span className="text-xs font-medium text-rose-700">Structure</span>
                </div>
              </div>

              {/* NGL Viewer Node (customNode — renders a protein/structure file in 3D) */}
              <div
                draggable
                onDragStart={(e) => {
                  e.dataTransfer.setData('application/reactflow', JSON.stringify({
                    ...NGL_VIEWER_NODE,
                    source_code: NGL_VIEWER_SOURCE,
                  }));
                  e.dataTransfer.effectAllowed = 'move';
                }}
                className="p-2 border rounded cursor-move hover:shadow-md transition-all bg-amber-500/10 border-amber-500/20 hover:border-amber-500/40"
                style={{
                  backgroundImage: 'repeating-linear-gradient(135deg, transparent, transparent 6px, rgba(245, 158, 11, 0.08) 6px, rgba(245, 158, 11, 0.08) 12px)',
                }}
                title="Interactive NGL 3D viewer for protein structures (PDB, mmCIF, GRO, PQR, MOL2, SDF)"
              >
                <div className="flex items-center gap-1.5">
                  <Atom className="h-3.5 w-3.5 text-amber-700 shrink-0" />
                  <span className="text-xs font-medium text-amber-700">NGL Viewer</span>
                </div>
              </div>
            </div>
          </div>
