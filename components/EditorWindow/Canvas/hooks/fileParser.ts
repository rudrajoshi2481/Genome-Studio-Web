...
export const convertFlowNodesToReactFlow = (nodes: FlowNode[]): Node[] => {
  ...
  return nodes.map(node => ({
    id: node.id,
    // Use dynamicNode type for nodes with inputs/outputs
    type: node.data.inputs || node.data.outputs ? 'dynamicNode' : node.type,
    position: node.position,
    data: {
      ...node.data,
      // Ensure required fields for DynamicCustomNode
      ...
      source_code: node.data.source_code || ''
    },
    draggable: node.draggable !== false,
    selectable: node.selectable !== false,
    deletable: node.deletable !== false
